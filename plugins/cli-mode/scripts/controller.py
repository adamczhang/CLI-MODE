...
def build_parser():
    ...
    p = sub.add_parser('usage'); p.add_argument('--name')
    p = sub.add_parser('undo'); p.add_argument('--name')
    p = sub.add_parser('test'); p.add_argument('--command', dest='test_command')
    p = sub.add_parser('brief'); p.add_argument('--action', choices=('show', 'clear', 'add'), default='show')
    p.add_argument('--text')
...
def run(args, control=None):
    ...
    elif command == 'usage': result = control.usage_report(control.session_of(args.name))
    elif command == 'undo': result = control.undo(control.session_of(args.name))
    elif command == 'test': result = control.tests(args.test_command)
    elif command == 'brief': result = control.brief(args.action, args.text)
