...
Wait for Yes before activation.
...
With an active agent, `/cli menu` and `/cli model` without a choice (and their
`$` equivalents) run controller `settings --menu-output` and show the second-page
Agent Settings view for the bound agent. It shows the accepted model, effort,
access and progress mode using the shared renderer.
...
4 toggles activity progress. X runs `settings --dismiss`, closing only the
...
Explicit `/cli model <choice>` retains direct setting behavior while active.
When inactive, `/cli menu` and `/cli model`, including explicit choices and `$`
forms, reply exactly `CLI-MODE: Agent not activated. /CLI to setup`.
